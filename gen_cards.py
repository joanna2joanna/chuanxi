...
BODY_STREAM = [
  ...
  ('h2', '高原的秩序'),
  ('lead', '海拔决定秩序。'),
  ('p', '进川西先认海拔。<strong>3000米是一道门槛</strong>，多数人的高反从这里开始。行程按海拔排，不按景点排：一天的路程控制在两三小时山路，下午留出空当看天，天气不对就提前收车。景点不会消失，路况和天气才是当天的变量。'),
  ('p', '窗口期。<strong>5到6月、9到10月中是全年最好走的时段</strong>。雨季6到8月以夜雨为主，白天多数能走。冬季也能走，但救援半径长，一台车陷在无人垭口，代价远超省下的时间。'),
  ('p', '防高反。从低到高慢慢爬，头两天不在3500米以上过夜。典型信号是头痛、睡不着、没胃口，多数人两三天能适应。乙酰唑胺遵医嘱提前吃，布洛芬缓解头痛。红景天降不了高反发生率，只缓解症状。高原低血糖常见，头晕乏力冲杯糖水能缓，补的是血糖不是氧。指夹式血氧仪测血氧，<strong>掉到90%以下就停下</strong>。夜里头痛加重、白天精神很差、走路发飘，是危险信号，撤到低海拔。'),
  ('p', '补给认准县城。<strong>进县城加满油</strong>，一离开县城就默认补给消失，往深处走，一律当天往返。离线地图出发前下好，<strong>自救物资备足，最好一直用不上</strong>。冬季走，车上常备防滑链。'),
  ('h2', '山河的地图'),
